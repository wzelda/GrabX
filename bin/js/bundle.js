(function () {
    ...
    class RigidObject {
        ...
        get IsStop() {
            return this.CurState == StateConfig.STOP;
        }
        getScript(script) {
            if (this.Obj)
        ...
        changeModel(path) {
            if (!path || this._modelPath == path)
                return;
            ObjectProxy.changeModel(this.Obj, this._modelPath, path);
            this._modelPath = path;
        }
        changeObj(key) {
            PoolManager.recover(key, this.Obj);
            this.Obj = ObjectProxy.getObj(key);
        }
        setPosition(pos) {
            if (this.Obj)
    ...
    class ObjectProxy {
        static createHand() {
            let hand = SceneManager.CurScene.addChild(new Laya.MeshSprite3D(Laya.PrimitiveMesh.createBox(ObjectConfig.HAND_SIZE.x, ObjectConfig.HAND_SIZE.y, ObjectConfig.HAND_SIZE.z)));
            this.addPhysics(hand, ObjectConfig.HAND_SIZE);
            return hand;
        }
        static createDesk() {
            let desk = SceneManager.CurScene.addChild(new Laya.MeshSprite3D(Laya.PrimitiveMesh.createBox(ObjectConfig.DESK_SIZE.x, ObjectConfig.DESK_SIZE.y, ObjectConfig.DESK_SIZE.z)));
        ...
        static getObj(key) {
            switch (key) {
                case PoolType.Hand:
                    return PoolManager.getObjByFunc(PoolType.Hand, this.createHand.bind(this));
                case PoolType.Desk:
                    return PoolManager.getObjByFunc(PoolType.Desk, this.createDesk.bind(this));
            }
        }
        static addPhysics(target, size) {
            if (!target || !size)
                return;
            let rigidBody = target.addComponent(Laya.Rigidbody3D);
            rigidBody.colliderShape = new Laya.BoxColliderShape(size.x, size.y, size.z);
            rigidBody.gravity = Laya.Vector3._ZERO;
            rigidBody.isTrigger = true;
            rigidBody.isKinematic = true;
        }
        static addScript(rigidObj, script) {
            if (!rigidObj || !script)
                return;
            return rigidObj.Obj.addComponent(script);
        }
        static changeModel(oldModel, oldPath, newPath) {
            if (!oldModel || !oldModel || !newPath || oldPath == newPath)
                return;
            if (oldModel) {
                PoolManager.recover(oldPath, oldModel);
            }
            let model = PoolManager.getItem(newPath);
            if (model instanceof Laya.MeshSprite3D) {
                oldModel = model;
            }
            else {
                SpawnManager.Load3dModel(newPath, (mdata) => {
                    oldModel = mdata.msp;
                }, this);
            }
        }
    ...
    class ObjectState {
        constructor(state, onUpdate, onEnter, onExit) {
            this.State = state;
    ...
    class GrabLogic extends EventDispather {
        ...
        onAwake() {
            ...
            this.DeskClass = new RigidObject(ObjectProxy.getObj(PoolType.Desk), new ObjectState(StateConfig.MOVE_FORWARD, this.deskDown.bind(this)), new ObjectState(StateConfig.DESK_LEAVE, this.deskLeave.bind(this)), new ObjectState(StateConfig.DESK_ENTER, this.deskEnter.bind(this)));
            this.DeskClass.setPosition(ObjectConfig.DESK_POS);
            this.HandClass = new RigidObject(ObjectProxy.getObj(PoolType.Hand), new ObjectState(StateConfig.MOVE_FORWARD, this.handForward.bind(this)), new ObjectState(StateConfig.MOVE_BACK, this.handBack.bind(this)), new ObjectState(StateConfig.BACK_PASSED, this.handBack.bind(this)));
            this.HandClass.setPosition(ObjectConfig.HAND_POS);
        ...
        addCollisionScript() {
            this.deskScript = this.DeskClass.addScript(DeskCollisionScript);
            this.deskScript.kinematicSprite = this.HandClass.Obj;
            this.handScript = this.HandClass.addScript(HandCollisionScript);
        ...
        resetVec() {
            this.Vdir.x = ObjectConfig.DESK_POS.x;
            this.Vdir.y = ObjectConfig.DESK_POS.y;
            this.Vdir.z = ObjectConfig.DESK_POS.z;
        }
        doKnock(loop = false) {
            this.resetVec();
            this.timeLine.reset();
        ...
        addStay(_stayTime) {
            ...
            this.timeLine.addLabel("stay", 0).to(this.Vdir, { y: ObjectConfig.DESK_POS.y }, _stayTime, null, 0);
        }
        addKnock(_deltaTime) {
            _deltaTime = _deltaTime ? _deltaTime * 1000 : 0;
            this.timeLine
                .to(this.Vdir, { y: ObjectConfig.DESK_END_POS.y }, 200, null, _deltaTime)
                .to(this.Vdir, { y: ObjectConfig.DESK_POS.y }, 200, null, 0);
        }
        restart() {
            this.deskScript.clearStatus();
            this.HandClass.changeState(StateConfig.IDEL);
            this.moveDesk();
            this.resetHand();
        }
        newLevel() {
            this.DeskClass.changeState(StateConfig.DESK_LEAVE);
        }
        moveDesk() {
            this.DeskClass.changeState(StateConfig.MOVE_FORWARD);
            this.doKnock(true);
        }
        resetDesk() {
            this.DeskClass.setPosition(ObjectConfig.DESK_POS);
        ...
        stopDesk() {
            this.timeLine.pause();
            this.DeskClass.changeState(StateConfig.STOP);
        }
        deskDown() {
        ...
        deskUp() {
            ...
            if (this.DeskClass.Position.y >= ObjectConfig.DESK_POS.y) {
                this.DeskClass.changeState(StateConfig.MOVE_FORWARD);
            }
        }
        deskEnter() {
            this.DeskClass.movePos(-0.1);
            if (this.DeskClass.Position.x <= ObjectConfig.DESK_POS.x) {
                this.moveDesk();
            }
        }
        onDeskDisappear() {
            this.DeskClass.changeObj(PoolType.Desk);
            this.DeskClass.setPosition(ObjectConfig.DESK_ENTER_POS);
            this.DeskClass.changeState(StateConfig.DESK_ENTER);
        }
        deskLeave() {
            if (!this.IsInited)
                return;
        ...
        moveHand() {
            if (!this.IsInited)
                return;
            if (this.HandClass.IsStop)
                return;
            if (this.HandClass.CurState == StateConfig.IDEL) {
                this.HandClass.changeState(StateConfig.MOVE_FORWARD);
            }
        }
        handForward() {
            if (!this.IsInited)
                return;
            this.HandClass.movePos(ObjectConfig.SPEED_HAND * TimerManager.frameDelta);
            if (this.HandClass.Position.x >= ObjectConfig.HAND_END_POS.x) {
                this.HandClass.changeState(StateConfig.MOVE_BACK);
            }
        }
        addPoint() {
            if (this.DeskClass.CurState == StateConfig.DESK_LEAVE || this.DeskClass.CurState == StateConfig.DESK_ENTER) {
                return;
            }
            PlayerData.Point += 100;
            console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>得分：", PlayerData.Point);
            if (PlayerData.Point >= 300) {
                this.newLevel();
            }
        }
        onReachFinish() {
        ...
        handBack() {
            if (!this.IsInited)
                return;
            if (this.HandClass.Position.x <= ObjectConfig.HAND_POS.x) {
                this.onReachFinish();
                return;
            }
            if (this.HandClass.Position.x < ObjectConfig.DESK_POS.x) {
                this.HandClass.changeState(StateConfig.BACK_PASSED);
            }
            this.HandClass.movePos(-ObjectConfig.SPEED_HAND * TimerManager.frameDelta);
        }
        resetHand() {
        ...
        stopHand() {
            this.HandClass.changeState(StateConfig.STOP);
        }
