...
//秒数转为时：分：秒
export function ConvertTime(cd, ignoreHour?:boolean){
...
//设置非负数
export function SetNonnegative(num:number){
    if(num < 0){
        num = 0;
    }
...
//搜寻数组键值
export function searchArray(arr:Array<any>, param:string, value){
...
export function saveLocalJson(key:string, value){
    //可存储数组
    if(!value) return;
 ...
 */
export function JsCallJava(classPath:string, javaFunc:string, data?, widthBack?:boolean){
    if(!Laya.Browser.onAndroid) return;
...
//计算字符字节数--遍历法--效率较高
export function strByteLen(str:string){ 
    let byteLen = 0, len:number; 
    if(str && typeof str == 'string'){
        len = str.length;
        for(let i = 0; i < len; i++){
...
//深拷贝
export function deepCopy(src:object, target:object){
