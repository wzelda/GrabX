...
export class LoadingProgressController extends Core.Controller{
    public View:UI.LoadingProgressView;
    public Progress = 0;
    ...
    showWxLogin(){
        if(!Common.isMiniGame() || LocalConfig.IsWxAuth || !this.IsLoaded) return;
    ...
    tryClose(){
        ...
        if(Manager.VersionManager.Version == Config.VersionConfig.Develop){
            if(!LocalConfig.IsChoosedService) return;
        }
        ...
        if(!Config.UIConfig.LoginPackageLoaded) return;
